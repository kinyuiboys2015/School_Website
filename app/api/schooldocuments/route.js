...
const cleanDocumentResponse = (document) => {
  ...
  return {
    ...
    feesBoardingAnnualAmount: totalFeeAmount(document.feesBoardingDistributionJson),
...
export async function POST(req) {
  try {
    ...
    // The SchoolDocument schema stores fee totals inside distribution JSON.
    const feesBoardingAnnualAmount = formData.get('feesBoardingAnnualAmount');

    if (feesBoardingAnnualAmount) {
      const amount = parseFloat(feesBoardingAnnualAmount);
      if (Number.isFinite(amount) && amount > 0) {
        updateData.feesBoardingDistributionJson = [
          {
            id: "boarding-total",
            name: "Boarding annual fees",
            description: "Total annual boarding fees",
            amount,
            mandatory: true,
          },
        ];
        console.log('✅ Boarding annual amount stored in fee distribution JSON:', amount);
      }
    }
