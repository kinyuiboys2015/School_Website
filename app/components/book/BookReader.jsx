...
const BookReader = ({ issue, onClose }) => {
    ...
    if (navigator.share) {
      try {
        await navigator.share({
          title: issue.title,
          text: `Reading ${issue.title} from Kinyui Boys Senior School.`,
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-hidden bg-[#160804] text-white">
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_top_left,rgba(245,158,11,0.24),transparent_28%),radial-gradient(circle_at_bottom_right,rgba(124,45,18,0.42),transparent_34%)]" />
      <div className="absolute inset-0 bg-[linear-gradient(120deg,rgba(255,255,255,0.045)_0,transparent_18%,transparent_82%,rgba(251,191,36,0.08)_100%)]" />
      <div className="absolute inset-y-0 left-0 hidden w-[88px] border-r border-orange-200/10 bg-[#2a1206]/55 backdrop-blur-xl xl:block" />
      ...
      <div className="reader-shell relative flex h-full flex-col xl:grid xl:grid-cols-[88px_340px_minmax(0,1fr)]">
          ...
          <div className="flex flex-col items-center gap-4 text-white/65">
            <button
              ...
              className={`flex h-12 w-12 items-center justify-center rounded-full border transition ${
                isLiked ? "border-amber-300/70 bg-amber-500/15 text-amber-100" : "border-white/10 bg-white/[0.05]"
              }`}
            >
            ...
            <button
              ...
              className={`flex h-12 w-12 items-center justify-center rounded-full border transition ${
                isBookmarked ? "border-amber-300/70 bg-amber-500/15 text-amber-100" : "border-white/10 bg-white/[0.05]"
              }`}
            >
        ...
        <aside className="hidden lg:flex lg:flex-col border-t border-orange-200/10 bg-[#2a1206]/50 backdrop-blur-xl lg:border-r lg:border-t-0 lg:order-2 xl:order-2">
          <div className="flex items-center justify-between border-b border-orange-200/10 px-4 py-4">
            <div>
          ...
          <div className="max-h-full overflow-y-auto px-4 py-5 lg:h-full lg:px-5 lg:py-6">
            <div className="rounded-[1.5rem] border border-amber-200/10 bg-[linear-gradient(160deg,rgba(83,34,10,0.98),rgba(31,12,4,0.99))] p-4 shadow-[0_24px_80px_rgba(0,0,0,0.3)]">
              ...
              <div className="mt-4">
                <p className="text-[8px] font-extrabold uppercase tracking-[0.28em] text-amber-200">
                  Edition
                </p>
              ...
              <div className="mt-4 grid grid-cols-2 gap-2">
                ...
                <div className="rounded-lg border border-white/10 bg-white/[0.06] px-2 py-2">
                  <p className="text-[8px] font-extrabold uppercase tracking-wider text-white/50">Views</p>
                  <p className="mt-1 text-xs font-black text-white">{issue.views || "Live"}</p>
                </div>
                <div className="rounded-lg border border-white/10 bg-white/[0.06] px-2 py-2">
                  <p className="text-[8px] font-extrabold uppercase tracking-wider text-white/50">Likes</p>
                  <p className="mt-1 text-xs font-black text-white">{likesCount}</p>
                </div>
              </div>
            </div>

            <div className="mt-4 rounded-[1.5rem] border border-amber-200/10 bg-[#1f0c04]/45 p-4">
              ...
              <div className="space-y-2 flex flex-col">
                <button
                  ...
                  className={`w-full flex items-center justify-between rounded-lg border px-3 py-2 text-left transition text-xs ${
                    isLiked ? "border-amber-300/50 bg-amber-500/15" : "border-white/10 bg-white/[0.03]"
                  }`}
                >
                  <span className="font-black text-white">Like</span>
                  <Heart className="h-4 w-4 text-amber-200" fill={isLiked ? "currentColor" : "none"} />
                </button>

                <button
                  onClick={handleBookmark}
                  className={`w-full flex items-center justify-between rounded-lg border px-3 py-2 text-left transition text-xs ${
                    isBookmarked ? "border-amber-300/50 bg-amber-500/15" : "border-white/10 bg-white/[0.03]"
                  }`}
                >
                  <span className="font-black text-white">Bookmark</span>
                  {isBookmarked ? (
                    <BookmarkCheck className="h-4 w-4 text-amber-200" />
                  ) : (
                    <Bookmark className="h-4 w-4 text-amber-200" />
                  )}
                </button>
        ...
        <main className="order-1 flex min-h-0 flex-col lg:order-2 flex-1">
          {/* Top Controls */}
          <div className="border-b border-orange-200/10 bg-[#2a1206]/45 px-3 py-3 backdrop-blur-xl sm:px-4 lg:px-6">
            <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
              <div className="flex items-center gap-2 sm:gap-3 min-w-0">
                <div className="flex h-10 w-10 sm:h-12 sm:w-12 items-center justify-center rounded-xl sm:rounded-2xl border border-white/10 bg-white/[0.06] shrink-0">
                  <BookOpen className="h-5 w-5 sm:h-5 sm:w-5 text-amber-200" />
                </div>
              ...
              <div className="flex flex-wrap items-center gap-2 sm:gap-3">
                ...
                >
                  ...
                  <ChevronDown className="h-3 w-3 sm:h-4 sm:w-4 text-amber-200" />
                </button>
              </div>
            </div>
            ...
            <div className="mt-2 h-1.5 overflow-hidden rounded-full bg-white/8">
              <motion.div
                className="h-full rounded-full bg-[linear-gradient(90deg,#7c2d12,#f59e0b,#fde68a)]"
          ...
          >
            ...
            <AnimatePresence>
              {showReaderTip && (
                ...
                >
                  <div className="rounded-full border border-amber-200/15 bg-[#2a1206]/92 px-5 py-3 text-xs font-bold uppercase tracking-[0.22em] text-white/82 shadow-[0_20px_50px_rgba(0,0,0,0.3)] backdrop-blur-xl">
            ...
            <div className="relative mx-auto flex w-max min-w-full flex-col items-center gap-6 pb-10">
              ...
              <Document
                ...
                loading={
                  <div className="flex min-h-[60vh] flex-col items-center justify-center rounded-[2rem] bg-[#fff7ed] px-8 py-20 text-center text-[#2a1206]">
                    <div className="flex h-16 w-16 items-center justify-center rounded-full bg-[#7c2d12] text-white">
                      <Loader2 className="h-7 w-7 animate-spin" />
                    </div>
                    <p className="mt-6 text-sm font-extrabold uppercase tracking-[0.3em] text-[#7c2d12]">
                      Building Reader
                    </p>
                ...
                error={
                  <div className="flex min-h-[60vh] flex-col items-center justify-center rounded-[2rem] bg-[#fff7ed] px-8 py-20 text-center text-[#2a1206]">
                    ...
                    <button
                      ...
                      className="mt-6 inline-flex items-center gap-2 rounded-full bg-[#7c2d12] px-5 py-3 text-sm font-black text-white"
                    >
                      Retry Reader
                ...
                {Array.from({ length: numPages || 0 }, (_, index) => {
                  ...
                  return (
                    <div
                      ...
                      className="relative mb-6 scroll-mt-5 overflow-hidden rounded-[1rem] border border-amber-200/20 bg-[#fff7ed] shadow-[0_20px_60px_rgba(0,0,0,0.38)] sm:rounded-[1.5rem] sm:shadow-[0_35px_110px_rgba(0,0,0,0.48)] lg:rounded-[2rem]"
                    >
                      <div className="absolute left-3 top-3 z-10 rounded-full bg-[#2a1206]/85 px-3 py-1 text-[10px] font-black uppercase tracking-[0.2em] text-white/85 backdrop-blur">
                        Page {pageNumber}
                      </div>
                      <Page
          ...
          <div className="border-t border-orange-200/10 bg-[#2a1206]/45 px-3 py-3 backdrop-blur-xl sm:px-4 lg:px-6">
            <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
                ...
                <button
                  ...
                  className={`inline-flex items-center gap-1 rounded-full px-3 py-2 text-xs sm:text-sm font-black transition ${
                    !numPages || currentPage >= numPages || isFlipping
                      ? "cursor-not-allowed border border-white/8 bg-white/[0.03] text-white/30"
                      : "bg-amber-500 text-[#2a1206] shadow-[0_14px_28px_rgba(245,158,11,0.28)]"
                  }`}
                >
        ...
        {showPageJump && (
          ...
          >
            <motion.div
              ...
              className="w-full max-w-sm rounded-[1.5rem] sm:rounded-[2rem] border border-amber-200/15 bg-[#2a1206] p-4 sm:p-6 text-white shadow-[0_30px_90px_rgba(0,0,0,0.38)]"
            >
              ...
              <form onSubmit={handleJumpSubmit} className="mt-4 sm:mt-6">
                <input
                  ...
                  className="w-full rounded-[1rem] border border-white/10 bg-white/[0.05] px-4 py-3 sm:py-4 text-base sm:text-lg font-bold text-white outline-none placeholder:text-white/35 focus:border-amber-300/60"
                  autoFocus
                />
                ...
                <div className="mt-4 sm:mt-5 flex flex-col gap-2 sm:flex-row sm:gap-3">
                  ...
                  <button
                    type="submit"
                    className="flex-1 rounded-lg sm:rounded-full bg-amber-500 px-4 py-2 sm:py-3 text-xs sm:text-sm font-black text-[#2a1206]"
                  >
                    Go
                  </button>
                </div>
        ...
        <div className="pointer-events-none absolute inset-0 z-40">
          <div
            className={`absolute inset-0 bg-gradient-to-r ${
              direction > 0 ? "from-amber-500/10 to-transparent" : "from-transparent to-amber-500/10"
            }`}
          />
        </div>
      )}
